...
describe('Employee zone e2e Tests', () => {
  beforeEach(() => {
  ...
  it('should display the employee section', () => {
    cy.contains('h1', 'Intranet').should('be.visible');
  });
  ...
  it('should create a new film when the form is valid', () => {
    ...
    cy.get('input[formControlName="filmTitle"]').type(`New Film ${newDay}`);
    cy.get('textarea[formControlName="filmDescription"]').type('Description du new film.');
    cy.get('input[formControlName="filmImg"]').type('https://example.com/new-film.webp');
    ...
    cy.get('[data-cy="film-table"]').contains('td', `New Film ${newDay}`).should('be.visible');
  });
  ...
  it('should delete the film created in the test', () => {
    ...
    cy.get('mat-dialog-container').within(() => {
      ...
      cy.contains(`Êtes-vous sûr de vouloir désactiver le film "New Film ${newDay}" ?`).should(
        'be.visible',
      );
  ...
  it('all the required fields to create a new room are present and button not active', () => {
    cy.get('.cinema-section mat-form-field').contains('Choisir un cinéma').click();
    cy.get('mat-option').not('[aria-disabled="true"]').eq(2).click();
    cy.wait(2000); // Wait for rooms and screenings to load
    cy.get('.add-room-button button').should('contain.text', 'Ajouter une Salle').and('be.visible');
